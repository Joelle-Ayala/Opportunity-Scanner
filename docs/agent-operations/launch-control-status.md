...
- The latest independently verified launch feature release is `5d71e5e`; read `/api/health` after every deployment for the exact live release.
...
- The live release adds verified first-party analytics readiness, explicit fail-closed subscription capacity, customer dashboard improvements, and additional paid-launch protections.

## Launch board

| Workstream | Status | Current evidence / decision | Next action |
|---|---|---|---|
| Production demo release | **Live / GO** | Feature release `5d71e5e` is live; production is demo-ready and the core routes return 200 with no recent Vercel runtime errors. | Preserve the live demo while completing the controlled paid launch gate. |
...
1. Configure GoDaddy MX records for `support@opportunityscanner.ai` and confirm the inbox can send and receive; the public domain currently returns no MX records.
...
- Latest verified launch feature release: `5d71e5e`; `/api/health` is the source of truth for the exact deployment release.
...
- Deployment proof: Vercel reported the feature release ready on the production domains and `/api/health` returned release `5d71e5ec907c` with demo and analytics ready.

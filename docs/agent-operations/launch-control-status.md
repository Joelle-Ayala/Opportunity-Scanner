...
**Ready for production setup and controlled live-domain verification, but not for external beta invitations or paid collection yet.** The launch-hardening bundle, workflow contract tests, connector reliability work, resource/article system, industry and solution content clusters, related-content UX, and gated lead-magnet acquisition flow are locally verified. Production hosting, Supabase schema/setup, required environment variables, deployment approval, fresh live-domain evidence, safe webhook policy, and the paid-pilot commercial decision still require founder/Chief of Staff action.
...
| Lead-magnet acquisition flow | Product Strategy + Front-End + Back-End | **Done locally / production schema pending** | Two source-backed guides have verified PDFs and landing pages; capture validates and persists only allowlisted guide requests; marketing consent is optional and false by default; users receive immediate browser access; PII is not placed in analytics or public errors. | Contract passes 12/12; desktop and 390px browser flows pass; PDFs render and exclude internal production notes. Production needs the new `lead_magnet_captures` schema and required Supabase environment. | Apply schema in production, verify row-level security, run one real capture/download, and add abuse protection before broad promotion. |
...
2. **Configure production persistence and environment.** Install the full Supabase schema, including `lead_magnet_captures`, and configure required environment variables without exposing secret values.
...
4. **Run the three-company and acquisition domain pass.** Fresh-scan Reparel, Jammcard, and SchoolGig; inspect desktop/mobile, free/full gating, Opportunity Action Table, source credibility, connector-run status, hidden debug data, failure behavior, and stored normalized fields. Submit one guide capture and verify consent, persistence, and PDF access. Rerun the evaluator.
...
- Launch environment check: 0/5 required values available locally; production setup still needs OpenAI, Supabase URL/service role, report access code, and admin code.
